...
{
   NSNumber *theNumber = [NSNumber numberWithInteger:theID];
   objc_setAssociatedObject(self, @selector(theID), theNumber, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}
...
- (NSInteger)theID
{
    NSNumber *theNumber = objc_getAssociatedObject(self, @selector(theID));
    if (!theNumber)
    {
        theNumber = [NSNumber numberWithInteger:-1];
        objc_setAssociatedObject(self, @selector(theID), theNumber, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    }
